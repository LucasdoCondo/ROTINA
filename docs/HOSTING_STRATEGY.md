...
> ⚠️ **Documento de referência (legado).** Copiado do branch `master` para preservar
> o histórico da decisão de hospedagem (free tiers, Vercel/Neon/Railway/Render,
> Redis/Upstash, DNS/Resend). Ele descreve o sistema JavaScript ativo em produção
> e **contém alternativas que não são mais usadas** neste branch.
> Para a arquitetura vigente do branch `main` (TypeScript) use
